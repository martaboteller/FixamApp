...
export class UsersService {
  ...
  filterUserByUid(uid: string): string {
    try {
      if (this.usersFromFirebase != null && this.usersFromFirebase.length > 1) {
        var userN = this.usersFromFirebase.filter((user) => user.uid === uid)[0]
          .username;
        return userN;
      }
    } catch (e) {
  ...
  uploadAvatar(image: File) {
    ...
    return new Promise((resolve, reject) => {
      const pictureRef = this.storage.ref(
        'userImages/' + this.authService.getToken() + '/' + imageName
      );
  ...
  deleteAvatar(user): Observable<any> {
    const pictureRef = this.storage.ref(
      'userImages/' + this.authService.getToken() + '/' + user.avatarFilename
    );
